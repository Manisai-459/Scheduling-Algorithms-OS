...
bool comparewthlb(const struct ready_queue &x, const struct ready_queue &y)
{
    if (x.bt == y.bt)
    {
        if (x.a == y.a)
...
void Sjf(vector<struct processes> ar, int n)
{
    ...
    while (rq.size() != 0)
    {
        ...
        while (1)
        {
            ...
            for (; i < n; i++)
            {
                ...
                {
                    rq.push_back(make_obj(ar[i]));
                    sort(rq.begin(), rq.end(), comparewthlb);
                    if (ar[i].id == rq[0].id)
                        x = 1;
                }
                else
    ...
    cout << "+\t\tBt\t\tAt\t\tTAT\t\tWt\t\tEt" << endl;
    for (int i = 0; i < n; i++)
        cout << "p" << ar[i].id << "\t\t" << ar[i].b << "\t\t" << ar[i].a << "\t\t" << ar[i].tat << "\t\t" << ar[i].w << "\t\t" << ar[i].et << endl;
